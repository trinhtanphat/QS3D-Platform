...
| recognition-profile catalog/persistence/editor | DONE_SHARED contract + DONE_BRX editor + SOURCE_READY_HOST AutoCAD #60 / PR #61 | Platform schema + host UI; native qualification pending |
| MEP rule authoring (duct/pipe/cable/fittings/equipment) | PARTIAL | Platform quantity/rule engine + host profile UI |
| MEP quantity-group -> BQ mapping | DONE_SHARED / PR #17 | Platform canonical projection with explicit Count/Length/Area/Volume basis |
| MEP BQ -> exact cost-rate projection | DONE_SHARED / PR #17 | Platform canonical exact item+unit+currency rate join |
| native/report rendering of MEP BQ/cost | PARTIAL | consuming host table/XLSX/CSV/palette surfaces |
...
| AutoCAD modeless MEP review/profile editor | SOURCE_READY_HOST / PENDING_NATIVE — AutoCAD #60 / PR #61, exact `cef52d0d…`, CI #218 green | AutoCAD UI/configuration over shared profile contracts |
...
- **AutoCAD native adapter:** issue #58 / stacked PR #59 is source-ready at `96cfa7b4e4b64b3e07f508034ac79d464731d261`. CI #204 / run `31872689952` passed Core smoke, AutoCAD 2021/net48, 2025-2026/net8 and 2027/net10 builds, parity/architecture/security/acceptance guards, packaging/provenance, Setup install/uninstall and signing-plumbing smoke. It is not merged and not native-qualified.
- **AutoCAD review/profile:** issue #60 / stacked PR #61 is source-ready at `cef52d0d468aef3696ab7e627a17d1140ef3a45f`. CI #218 / run `31873799933` passed the same multi-target build/package/security matrix plus the dedicated review/profile guard. The palette/profile source is not merged and real AutoCAD palette/DPI/multi-DWG/profile-restart behavior remains `PENDING_NATIVE`.
...
- **Shared MEP -> BQ/cost:** Platform issue #16 / stacked PR #17 is source-ready at `eb36f886083202ebd4190ae18edd3b25227544fa`; Platform CI #122 / run `31873450699` passed exact-head shared validation before this documentation synchronization commit.
- AutoCAD and standalone host lanes pin the same exact shared Platform candidate `e029d4ba0de6ffe80575f7aed96affa1db1b9b33`, preventing silent drift while PR #15 remains unintegrated.

## 6. TBQ-style digital BQ / estimating / cost management

Shared clean-room contracts delivered across #13 and #16 cover:
...
21. MEP quantity-group -> BQ item mapping with deterministic priority, optional kind/system/specification/region filters and explicit Count/Length/Area/Volume basis.
22. exact BQ unit compatibility (`ea`, `m`, `m2`, `m3`), source-group contribution provenance and fail-closed mapping ambiguity.
23. MEP BQ -> `CostRateBuildUp` pricing by exact item code + unit + currency, with missing/duplicate rate refusal and checked decimal totals.
...
### Wave B — BricsCAD review UX + shared MEP commercial projection
...
- [x] shared MEP quantity -> BQ -> exact cost-rate projection (#16 / PR #17);
- [ ] wire the shared MEP BQ/cost projection into BricsCAD native table/XLSX/CSV/report UI without duplicating Platform rules;
- [ ] licensed V25/V26 exact-SHA qualification for the integrated review/profile wave.

The delivered BricsCAD source wave is integrated at `03816d9179a1927afa1450f5c821d4a73d0ddc53`; that is source integration evidence only, not native runtime qualification. Shared MEP BQ/cost projection is source-ready on Platform PR #17 and still awaits repository-authorized integration.
...
- [x] add AutoCAD modeless MEP review palette and hardened recognition-profile persistence/editor (#60 / PR #61); native palette/profile matrix remains pending;
...
- [ ] wire shared MEP BQ/cost projections into consuming host report/export UI where product UX requires it;
