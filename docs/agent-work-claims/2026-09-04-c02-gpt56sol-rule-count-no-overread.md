# Work claim — C02 counted-source materialization no-overread
...
## Reserved scope
Fail closed when authoritative-count quantity-rule factor/catalog or quantity-fact sources yield beyond advertised Count, before accessing `IEnumerator.Current` for the overrun item.

## Expected surfaces
- `src/QS3D.Platform.Quantity/QuantityRules.cs`
- `src/QS3D.Platform.Quantity/QuantityModel.cs`
- focused deterministic smoke(s) under `tests/QS3D.Platform.SmokeTests/`
...
## Validation plan
- TDD RED with counted hostile rule/fact enumerables whose overrun `Current` access throws;
...
- preserve semantic generation replay, provenance and deterministic ordering;
